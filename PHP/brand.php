...
<div class="menu_overlay" id="globalMenu" role="navigation">
    <nav>
        <?php foreach ($favorite_brands as $index => $fav_brand): ?>
            <a href="brand.php?id=<?= htmlspecialchars($fav_brand['id']) ?>"
               class="bland"
               style="--index: <?= $index ?>; top: <?= 75 + ($index % 4) * 50 ?>px; left: <?= 173 - ($index % 4) * 60 ?>px;">
                <?= htmlspecialchars($fav_brand['name']) ?>
            </a>
            <?php if (($index + 1) % 3 === 0): ?>
                <br>
            <?php endif; ?>
